            ...
            for λ in self:
                if isinstance(λ, λ_chain):
                    ...
                    match mode:
                        ...
                        case 'f':
                            set_or_add(args, 0, nth_or_empty(list(zip(*[i for i in zip(*args) if λ(*i, **kwargs)])), barg))
                        case 's':
                            z = zip(*args)
                            q = list(zip(*sorted(z, key=lambda x: λ(*x, **kwargs))))
                            set_or_add(args, 0, nth_or_empty(q, barg))
...
class Macro_transformer(NodeTransformer):
    ...
    def visit_Call(self, node):
        ...
        if isinstance(node, ast.Call) and hasattr(node, 'func'):
            ...
            # xyzw: special, io: extra
            keyargs = OrderedDict({i: Constant(None) for i in "xyzwio"} | {i.arg: i.value for i in node.keywords})
            node.args[0] = Lambda(
                body = node.args[0],
                args = arguments(
...
XSH.execer.parser.custom_tree_process = process_tree